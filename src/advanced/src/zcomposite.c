...
/* zcomposite.c - by Tom McReynolds, SGI */

/* Compositing images that include depth information. */

/* To use this program, you select a region of the "room" scene by
   rubber-banding the region with the left mouse button.  Once a
   region is selected, position the region with the middle mouse
   button.  The region will be composited using the region image and
   the region's depth information.  With the right mouse button, you
   can shift the region's depth values nearer or further away. */
...
motion(int x, int y)
{
  ...
  if (rubberbandmode) {
    wid = x - startx;
    ht = y - starty;
    if (overlaySupport) glutPostOverlayRedisplay();
  }
  if (depthmode)
    setRasterPosZ(y);
}
...
mouse(int button, int state, int x, int y)
{
  ...
  if (state == GLUT_DOWN)
    ...
    case GLUT_LEFT_BUTTON:  /* select an image */
      ...
      if (overlaySupport) glutShowOverlay();
      break;
    ...
    case GLUT_LEFT_BUTTON:
      ...
      if (overlaySupport) glutHideOverlay();
      wid = x - startx;
      ht = y - starty;
      if (wid < 0) {
        wid = -wid;
...
int main(int argc, char *argv[])
{
  ...
  gluDisk(obj, 0., 20., 20, 1);
  gluQuadricOrientation(obj, GLU_OUTSIDE);
  gluCylinder(obj, 20., 0., 60., 20, 20);
  glEndList();
  ...
  if (glutLayerGet(GLUT_OVERLAY_POSSIBLE)) {
    ...
    printf("Running without overlay rubber banding support.\n");
  }
  glutMainLoop();
